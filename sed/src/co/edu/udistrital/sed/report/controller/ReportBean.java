		...
		try {
			...
			if (this.idSelectedGrade == null || this.idSelectedGrade.equals(0L)) {
				addWarnMessage("Agregar Archivo",
					"Para poder subir el archivo es necesario que indique el grado al cual quiere subir la informaci�n.");
				return;
			} else {
			...
			if (event != null) {
				...
				if (inputFile != null) {
					...
					// Procesar Archivo Excel en busca de errores de inserci�n
					if (this.wbDegree != null) {
						...
						if (this.totalStudentList != null && this.properStudentList != null
							...
							if (this.controller.saveCalificationList(this.properStudentList, getUserSession() != null ? getUserSession()
								...
								addInfoMessage("Cargar archivo", "El archivo fu� leido y almacenado en la base de datos correctamente.");
							}

						} else {
							setFileError(true);
							addWarnMessage("Cargar archivo",
								"Se analiz� el archivo y se encontraron algunas fallas, por favor verifique el archivo e intente nuevamente.");
							return;
						}
						cleanVarList();
					} else {
